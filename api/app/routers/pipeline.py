...
def list_oportunidades(current_user: CurrentUser, repo: RepoDep, estado: str | None = None, q: str | None = None) -> list[OportunidadOut]:
...
def conversion_comercial(current_user: CurrentUser, repo: RepoDep) -> ConversionComercialOut:
    return ConversionComercialOut(**repo.conversion_comercial())


@router.post("", response_model=OportunidadOut, status_code=201)
def create_oportunidad(body: OportunidadIn, current_user: CurrentUser, repo: RepoDep, _: dict = require_permission("expedientes", "crear")) -> OportunidadOut:
...
def update_oportunidad(oportunidad_id: int, body: OportunidadUpdate, current_user: CurrentUser, repo: RepoDep, _: dict = require_permission("expedientes", "editar")) -> OportunidadOut:
...
def transicionar_oportunidad(oportunidad_id: int, body: OportunidadTransicion, current_user: CurrentUser, repo: RepoDep, _: dict = require_permission("expedientes", "editar")) -> OportunidadTransicionOut:
